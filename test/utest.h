...
UTEST_C_FUNC __declspec(dllimport) int __stdcall QueryPerformanceCounter(utest_large_integer*);
UTEST_C_FUNC __declspec(dllimport) int __stdcall QueryPerformanceFrequency(utest_large_integer*);
...
#if defined(__clang__)
#define UTEST_INITIALIZER_BEGIN_DISABLE_WARNINGS \
    _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wmissing-variable-declarations\"")
...
static UTEST_INLINE utest_int64_t utest_ns(void) {
    ...
    return UTEST_CAST(utest_int64_t, (counter.QuadPart * 1000000000) / frequency.QuadPart);
...
#endif
#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 201112L) && !(defined(__MINGW32__) || defined(__MINGW64__))
...
#define UTEST_SURPRESS_WARNING_BEGIN                                                            \
    __pragma(warning(push)) __pragma(warning(disable : 4127)) __pragma(warning(disable : 4571)) \
    ...
    UTEST_SURPRESS_WARNING_BEGIN do {                                                                       \
        _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wlanguage-extension-token\"") \
            _Pragma("clang diagnostic ignored \"-Wc++98-compat-pedantic\"")                                 \
                _Pragma("clang diagnostic ignored \"-Wfloat-equal\"") UTEST_AUTO(x) xEval = (x);            \
        UTEST_AUTO(y) yEval = (y);                                                                          \
        if(!((xEval)cond(yEval))) {                                                                         \
            _Pragma("clang diagnostic pop") UTEST_PRINTF("%s:%i: Failure\n", __FILE__, __LINE__);           \
...
#define EXPECT_STREQ(x, y)                                                            \
    UTEST_SURPRESS_WARNING_BEGIN do {                                                 \
        const char* xEval = (x);                                                      \
        const char* yEval = (y);                                                      \
        if(UTEST_NULL == xEval || UTEST_NULL == yEval || 0 != strcmp(xEval, yEval)) { \
            UTEST_PRINTF("%s:%i: Failure\n", __FILE__, __LINE__);                     \
...
#define EXPECT_STRNEQ(x, y, n)                                                                      \
    UTEST_SURPRESS_WARNING_BEGIN do {                                                               \
        ...
        if(UTEST_NULL == xEval || UTEST_NULL == yEval || 0 != UTEST_STRNCMP(xEval, yEval, nEval)) { \
            UTEST_PRINTF("%s:%i: Failure\n", __FILE__, __LINE__);                                   \
...
#define EXPECT_NEAR(x, y, epsilon)                                                     \
    UTEST_SURPRESS_WARNING_BEGIN do {                                                  \
        const double diff = utest_fabs(UTEST_CAST(double, x) - UTEST_CAST(double, y)); \
...
#define UTEST_F_SETUP(FIXTURE) static void utest_f_setup_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture)

#define UTEST_F_TEARDOWN(FIXTURE) \
    static void utest_f_teardown_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture)
...
#define UTEST_F(FIXTURE, NAME)                                                      \
    ...
    static void utest_f_##FIXTURE##_##NAME(int* utest_result, size_t utest_index) { \
    ...
    void utest_run_##FIXTURE##_##NAME(int* utest_result, struct FIXTURE* utest_fixture)

#define UTEST_I_SETUP(FIXTURE) \
    static void utest_i_setup_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture, size_t utest_index)

#define UTEST_I_TEARDOWN(FIXTURE) \
    static void utest_i_teardown_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture, size_t utest_index)

#define UTEST_I(FIXTURE, NAME, INDEX)                                                   \
    UTEST_EXTERN struct utest_state_s utest_state;                                      \
    static void utest_run_##FIXTURE##_##NAME##_##INDEX(int*, struct FIXTURE*);          \
    static void utest_i_##FIXTURE##_##NAME##_##INDEX(int* utest_result, size_t index) { \
        struct FIXTURE fixture;                                                         \
        memset(&fixture, 0, sizeof(fixture));                                           \
    ...
    void utest_run_##FIXTURE##_##NAME##_##INDEX(int* utest_result, struct FIXTURE* utest_fixture)
...
int utest_main(int argc, const char* const argv[]) {
    ...
    for(index = 1; index < UTEST_CAST(size_t, argc); index++) {
        ...
        if(0 == UTEST_STRNCMP(argv[index], help_str, strlen(help_str))) {
            ...
            goto cleanup;
        } else if(0 == UTEST_STRNCMP(argv[index], filter_str, strlen(filter_str))) {
            /* user wants to filter what test cases run! */
            filter = argv[index] + strlen(filter_str);
        } else if(0 == UTEST_STRNCMP(argv[index], output_str, strlen(output_str))) {
            ...
            return 0;
        } else if(0 == UTEST_STRNCMP(argv[index], enable_mixed_units_str, strlen(enable_mixed_units_str))) {
            enable_mixed_units = 1;
        } else if(0 == UTEST_STRNCMP(argv[index], random_order_with_seed_str, strlen(random_order_with_seed_str))) {
            seed =
                UTEST_CAST(utest_uint32_t, strtoul(argv[index] + strlen(random_order_with_seed_str), UTEST_NULL, 10));
            random_order = 1;
        } else if(0 == UTEST_STRNCMP(argv[index], random_order_str, strlen(random_order_str))) {
            ...
            // don't want).
            seed = UTEST_CAST(utest_uint32_t, ns >> 32) * 31 + UTEST_CAST(utest_uint32_t, ns & 0xffffffff);
            random_order = 1;
        }
    }
    ...
    if(random_order) {
        ...
        for(index = utest_state.tests_length; index > 1; index--) {
            ...
            const utest_uint32_t word = ((state >> ((state >> 28u) + 4u)) ^ state) * 277803737u;
            const utest_uint32_t next = ((word >> 22u) ^ word) % UTEST_CAST(utest_uint32_t, index);
    ...
    for(index = 0; index < utest_state.tests_length; index++) {
        ...
        printf("%s[ RUN      ]%s %s\n", colours[GREEN], colours[RESET], utest_state.tests[index].name);
        ...
        if(UTEST_TEST_FAILURE == result) {
            ...
            failed_testcases = UTEST_PTR_CAST(
                size_t*,
                utest_realloc(UTEST_PTR_CAST(void*, failed_testcases), sizeof(size_t) * failed_testcases_length));
            ...
            skipped_testcases = UTEST_PTR_CAST(
                size_t*,
                utest_realloc(UTEST_PTR_CAST(void*, skipped_testcases), sizeof(size_t) * skipped_testcases_length));
    ...
    printf("%s[  PASSED  ]%s %" UTEST_PRIu64 " tests.\n", colours[GREEN], colours[RESET], ran_tests - failed - skipped);
    ...
    if(0 != failed) {
        ...
        for(index = 0; index < failed_testcases_length; index++) {
            printf(
                "%s[  FAILED  ]%s %s\n", colours[RED], colours[RESET], utest_state.tests[failed_testcases[index]].name);
        }
    }
